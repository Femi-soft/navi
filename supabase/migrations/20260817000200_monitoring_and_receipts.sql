...
create table if not exists monitoring_checks (
  ...
  network varchar not null check (network in ('testnet','mainnet')),
  chain_id integer check (chain_id in (1952,196)),
  block_number varchar,
  status varchar not null check (status in ('healthy','degraded')),
